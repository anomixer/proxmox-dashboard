...
1. **Proxmox API Token の取得**
   ...
   - 新しい Token を作成（Token ID: token-name、「権限分離（Privilege Separation）」のチェックを外し、「追加（Add）」をクリック）
   - Token Secret を安全な場所に保存（Token ID: root@pam!token-name、「シークレット値をコピー（Copy Secret Value）」をクリック）